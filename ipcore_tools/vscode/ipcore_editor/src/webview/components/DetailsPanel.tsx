...
import { FIELD_COLORS, FIELD_COLOR_KEYS, getFieldColor } from '../shared/colors';
import {
    parseBitsRange,
    formatBits,
    repackFieldsFrom,
...
const DetailsPanel = React.forwardRef<DetailsPanelHandle, DetailsPanelProps>(
    ({ selectedType, selectedObject, selectionMeta, onUpdate }, ref) => {
        ...
        useEffect(() => {
            ...
            const tryInsertField = (after: boolean) => {
                ...
                if (after) {
                    ...
                    if (newLsb < 0 || newMsb >= regSize) {
                        ...
                        window.setTimeout(
                            () =>
                                fieldsErrorRef.current?.scrollIntoView({ block: 'nearest', behavior: 'smooth' }),
                            0
                        );
                        return;
                    }
            ...
            const onKeyDown = (e: KeyboardEvent) => {
                ...
                const isInFieldsArea =
                    !!fieldsFocusRef.current &&
                    !!activeEl &&
                    (activeEl === fieldsFocusRef.current || fieldsFocusRef.current.contains(activeEl));
                if (!isInFieldsArea) {
                    return;
                }
        ...
        useEffect(() => {
            ...
            const blocks = selectedObject?.address_blocks || selectedObject?.addressBlocks || [];
            ...
            const onKeyDown = (e: KeyboardEvent) => {
                ...
                const isInBlocksArea =
                    !!blocksFocusRef.current &&
                    !!activeEl &&
                    (activeEl === blocksFocusRef.current || blocksFocusRef.current.contains(activeEl));
                if (!isInBlocksArea) {
                    return;
                }
        ...
        useEffect(() => {
            ...
            const onKeyDown = (e: KeyboardEvent) => {
                ...
                const isInRegsArea =
                    !!regsFocusRef.current &&
                    !!activeEl &&
                    (activeEl === regsFocusRef.current || regsFocusRef.current.contains(activeEl));
                if (!isInRegsArea) {
                    return;
                }
        ...
        useEffect(() => {
            ...
            selectedType,
            selectedObject?.name,
            (selectedObject?.address_blocks || selectedObject?.addressBlocks || []).length,
        ]);
        ...
        useEffect(() => {
            ...
            // eslint-disable-next-line react-hooks/exhaustive-deps
        }, [selectedType, selectedObject?.name, (selectedObject?.registers || []).length]);
                ...
                <div className="flex flex-col w-full h-full min-h-0">
                    ...
                    <div className="flex-1 flex overflow-hidden min-h-0">
                        <div className="flex-1 vscode-surface border-r vscode-border min-h-0 flex flex-col">
                            ...
                            >
                                {fieldsInsertError ? (
                                    <div ref={fieldsErrorRef} className="vscode-error px-4 py-2 text-xs">
                                        {fieldsInsertError}
                                    </div>
                                ) : null}
                                <table className="w-full text-left border-collapse table-fixed">
                                    ...
                                    <tbody className="divide-y vscode-border text-sm">
                                        {fields.map((field, idx) => {
                                            ...
                                            const color = getFieldColor(field.name || `field${idx}`, field.bit_offset);
                                            const resetDisplay =
                                                        ...
                                                        >
                                                            <div className="flex items-center h-10">
                                                                <VSCodeDropdown
                                                                    ...
                                                                    onInput={(e: any) =>
                                                                        onUpdate(['fields', idx, 'access'], e.target.value)
                                                                    }
                                                                >
                                                        ...
                                                        >
                                                            <div className="flex items-center h-10">
                                                                <VSCodeTextArea
                                                                    ...
                                                                    onInput={(e: any) =>
                                                                        onUpdate(['fields', idx, 'description'], e.target.value)
                                                                    }
                                                                />
                                                            </div>
                                                        </td>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <VSCodeTextArea
                                                            ...
                                                            onInput={(e: any) =>
                                                                onUpdate(['addressBlocks', idx, 'description'], e.target.value)
                                                            }
                                                        />
                                                    </td>
                                                </tr>
