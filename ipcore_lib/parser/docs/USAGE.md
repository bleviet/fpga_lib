...
```bash
uv run pytest ipcore_lib/tests/test_vhdl_ai_parser.py -v
```
...
uv run pytest ipcore_lib/tests/test_vhdl_ai_parser.py::TestComplexExpressions -v

# Bus Interface Detection
uv run pytest ipcore_lib/tests/test_vhdl_ai_parser.py::TestBusInterfaceDetection -v
```

### Integration Tests (Live LLM)
Tests marked as `integration` will call the actual LLM provider.
```bash
uv run pytest ipcore_lib/tests/test_vhdl_ai_parser.py -v -m integration
```
