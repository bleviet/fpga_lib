...
|--------|----------|----------------|
| **Loader** | `ipcore_lib.driver.loader` | Parsing YAML, building object hierarchy. |
| **Bus** | `ipcore_lib.driver.bus` | Concrete bus implementations (e.g., `CocotbBus`). |
| **Runtime**| `ipcore_lib.runtime.register`| Generic `Register`, `BitField`, `AddressBlock` classes. |
