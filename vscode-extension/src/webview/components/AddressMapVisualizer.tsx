...
const AddressMapVisualizer: React.FC<AddressMapVisualizerProps> = ({
    ...
    const maxAddress = useMemo(() => {
        if (!blocks || blocks.length === 0) return totalAddressSpace;
    ...
    const visualGroups = useMemo(() => {
        ...
        if (totalBlockSize === 0) return groups.map(g => ({ ...g, widthPercent: 100 / groups.length }));
    ...
    return (
        <div className="w-full">
                            ...
                            >
                                ...
                                >
                                    <div className="font-bold">
                                        {group.name}
                                        <span className="ml-2 vscode-muted font-mono text-[11px]">[{toHex(group.start)}:{toHex(group.end)}]</span>
                                    </div>
                                    <div className="text-[11px] vscode-muted font-mono">
                                        {group.size < 1024 ? `${group.size}B` : group.size < 1048576 ? `${(group.size / 1024).toFixed(1)}KB` : `${(group.size / 1048576).toFixed(1)}MB`}
                                    </div>
                                </div>
                                <div className="flex w-full justify-center">
                                    <div className="text-center text-[11px] vscode-muted font-mono mt-1">{toHex(group.start)}</div>
                                </div>
                            </div>
                        );
                    })}
            ...
            <div className="mt-3 flex items-center justify-end gap-3">
                <div className="text-sm vscode-muted font-mono">Total Address Space: {toHex(maxAddress)}</div>
            </div>
        </div>
    );
};
