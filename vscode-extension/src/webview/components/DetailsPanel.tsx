        ...
        return (
            <div className="flex flex-col w-full h-full">
                ...
                <div className="flex-1 flex overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 border-b border-gray-200 w-24">Bit(s)</th>
                                    <th className="px-4 py-3 border-b border-gray-200 w-20">Access</th>
                                    <th className="px-4 py-3 border-b border-gray-200 w-24">Reset</th>
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-6 py-3 font-medium text-gray-900 flex items-center gap-2">
                                                <div className={`w-2.5 h-2.5 rounded-sm`} style={{ backgroundColor: color === 'gray' ? '#e5e7eb' : (colorMap && colorMap[color]) || color }}></div> {field.name}
                                            </td>
                    ...
                    <div className="w-96 bg-gray-50 flex flex-col shrink-0 overflow-y-auto">
                        <div className="p-6">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                                <span className="material-symbols-outlined text-[16px]">tune</span> Field Properties
                            </h3>
                            {selectedFieldIndex >= 0 && fields[selectedFieldIndex] ? (
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <VSCodeTextField
                                            ...
                                            onInput={(e: any) => onUpdate(['fields', selectedFieldIndex, 'name'], e.target.value)}
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-gray-500 mb-1">Bits</label>
                                        <VSCodeTextField
                                            className="w-full"
                                            value={toBits(fields[selectedFieldIndex])}
                                            readOnly
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <VSCodeDropdown
                                            value={fields[selectedFieldIndex].access || 'read-write'}
                                            onInput={(e: any) => onUpdate(['fields', selectedFieldIndex, 'access'], e.target.value)}
                                        >
                                    ...
                                    <div>
                                        <label className="block text-xs font-medium text-gray-500 mb-1">Reset Value</label>
                                        <VSCodeTextField
                                            className="w-full"
                                            value={fields[selectedFieldIndex].reset_value ?? ''}
                                            onInput={(e: any) => onUpdate(['fields', selectedFieldIndex, 'reset_value'], e.target.value)}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <VSCodeTextArea
                                            ...
                                            rows={3}
                                            onInput={(e: any) => onUpdate(['fields', selectedFieldIndex, 'description'], e.target.value)}
                                        />
                                    </div>
                                </div>
                            ) : (
                                <div className="text-gray-400 text-xs">Select a field to view/edit its properties.</div>
                            )}
                        </div>
                    </div>
                </div>
