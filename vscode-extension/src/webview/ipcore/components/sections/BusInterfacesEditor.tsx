...
function getEffectivePorts(
    ...
    return libraryPorts
        .filter(port => {
            if (port.presence === 'required') return true;
            if (port.presence === 'optional' && optionalPorts.includes(port.name)) return true;
            return false;
        })
        .map(port => ({
            ...port,
            width: widthOverrides[port.name] ?? port.width,
            direction: bus.mode === 'slave' && port.direction
                ? (port.direction === 'in' ? 'out' : port.direction === 'out' ? 'in' : port.direction)
                : port.direction,
        }));
}
...
export const BusInterfacesEditor: React.FC<BusInterfacesEditorProps> = ({ busInterfaces, busLibrary, imports, clocks = [], resets = [], onUpdate, highlight }) => {
    ...
    const [editingPortName, setEditingPortName] = useState<{ busIndex: number; portName: string } | null>(null);
    ...
    const [editingPortWidth, setEditingPortWidth] = useState<{ busIndex: number; portName: string } | null>(null);
    ...
    const toggleExpandAll = useCallback(() => {
        if (expandAll) setExpandedIndexes(new Set());
        else setExpandedIndexes(new Set(busInterfaces.map((_, i) => i)));
    ...
    const savePrefix = useCallback((index: number) => {
    ...
    const startEditPortName = useCallback((busIndex: number, portName: string, currentSuffix: string) => {
        setEditingPortName({ busIndex, portName });
        setDraftPortName(currentSuffix);
    }, []);

    const savePortName = useCallback((busIndex: number, portName: string) => {
        ...
        onUpdate(['busInterfaces', busIndex, 'portNameOverrides'], Object.keys(newOverrides).length > 0 ? newOverrides : undefined);
    ...
    // Width editing
    const startEditPortWidth = useCallback((busIndex: number, portName: string, currentWidth: number) => {
        setEditingPortWidth({ busIndex, portName });
        setDraftPortWidth(String(currentWidth));
    }, []);

    const savePortWidth = useCallback((busIndex: number, portName: string, defaultWidth: number) => {
        const bus = busInterfaces[busIndex];
        ...
        onUpdate(['busInterfaces', busIndex, 'portWidthOverrides'], Object.keys(newOverrides).length > 0 ? newOverrides : undefined);
    ...
    const startEditArrayField = useCallback((busIndex: number, field: ArrayField, currentValue: string | number) => {
        setEditingArrayField({ busIndex, field });
        setDraftArrayValue(String(currentValue));
    }, []);

    const saveArrayField = useCallback((busIndex: number) => {
        if (!editingArrayField) return;
    ...
    const toggleArray = useCallback((busIndex: number, hasArray: boolean) => {
        if (hasArray) {
            // Remove array
    ...
    const toggleOptionalPort = useCallback((busIndex: number, portName: string, currentlyEnabled: boolean) => {
        const bus = busInterfaces[busIndex];
        ...
        onUpdate(['busInterfaces', busIndex, 'useOptionalPorts'], newOptional.length > 0 ? newOptional : undefined);
    }, [busInterfaces, onUpdate]);

    // Update associated clock/reset
    const updateAssociation = useCallback((busIndex: number, field: 'associatedClock' | 'associatedReset', value: string) => {
        onUpdate(['busInterfaces', busIndex, field], value || undefined);
    }, [onUpdate]);

    // Get all optional ports from library
    const getOptionalPorts = useCallback((bus: BusInterface): BusPort[] => {
        if (!busLibrary || !bus.type || !busLibrary[bus.type]) return [];
        const libraryDef = busLibrary[bus.type];
    ...
    return (
        <div ref={containerRef} className="p-6 space-y-4" tabIndex={0} style={{ outline: 'none' }}>
            ...
            {busInterfaces.length === 0 ? (
                <div className="p-8 text-center rounded text-sm" style={{ background: 'var(--vscode-editor-background)', border: '1px solid var(--vscode-panel-border)', ...TEXT_STYLES.muted }}>
                ...
                <div className="space-y-2">
                    {busInterfaces.map((bus, index) => {
                        ...
                        return (
                            <div key={index} className="rounded overflow-hidden" style={{ border: '1px solid var(--vscode-panel-border)' }}>
                                {/* Header row */}
                                <div
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {editingBusField?.busIndex === index && editingBusField?.field === 'name' ? (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); cancelEditBusField(); }} className="px-1 py-0.5 rounded text-xs" style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-secondaryForeground)' }}>✗</button>
                                                </div>
                                            ) : (
                                                <span
                                        ...
                                        <div className="flex items-center gap-4 mt-1 text-sm" style={TEXT_STYLES.muted}>
                                            {bus.associatedClock && (
                                                <span title={`Port: ${clockInfo?.physicalPort || 'N/A'}\nFrequency: ${clockInfo?.frequency || 'N/A'}`} style={{ cursor: 'help' }}>
                                                    <span style={TEXT_STYLES.label}>Clock:</span> <span style={TEXT_STYLES.value}>{bus.associatedClock}</span>
                                                    {clockInfo?.frequency && <span style={TEXT_STYLES.muted}> ({clockInfo.frequency})</span>}
                                                </span>
                                            )}
                                            {bus.associatedReset && (
                                                <span title={`Port: ${resetInfo?.physicalPort || 'N/A'}\nPolarity: ${resetInfo?.polarity || 'N/A'}`} style={{ cursor: 'help' }}>
                                                    <span style={TEXT_STYLES.label}>Reset:</span> <span style={TEXT_STYLES.value}>{bus.associatedReset}</span>
                                                    {resetInfo?.polarity && <span style={TEXT_STYLES.muted}> ({resetInfo.polarity})</span>}
                                                </span>
                                            )}
                                ...
                                {expanded && (
                                    <div style={{ borderTop: '1px solid var(--vscode-panel-border)' }}>
                                        {/* Prefix editor */}
                                        <div className="px-4 py-2 flex items-center gap-3 text-sm" style={{ background: 'var(--vscode-editor-background)' }}>
                                            ...
                                            {editingPrefix === index ? (
                                                <div className="flex items-center gap-2 flex-1">
                                                    <input
                                                        ...
                                                        style={{ ...TEXT_STYLES.value, background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', color: 'var(--vscode-input-foreground)', outline: 'none' }}
                                                        placeholder="e.g., s_axi_"
                                                        autoFocus
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter') { e.preventDefault(); savePrefix(index); }
                                                            else if (e.key === 'Escape') { e.preventDefault(); cancelEditPrefix(); }
                                                        }}
                                                    />
                                                    <button onClick={() => savePrefix(index)} className="px-2 py-1 rounded text-sm" style={{ background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)' }}>Save</button>
                                                    <button onClick={cancelEditPrefix} className="px-2 py-1 rounded text-sm" style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-secondaryForeground)' }}>Cancel</button>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-2 flex-1">
                                                    <span style={TEXT_STYLES.value}>{bus.physicalPrefix || <span style={TEXT_STYLES.muted}>not set</span>}</span>
                                                    <button onClick={(e) => { e.stopPropagation(); startEditPrefix(index, bus.physicalPrefix || ''); }} className="p-1 rounded" title="Edit prefix">
                                        ...
                                        <div className="px-4 py-2 text-sm grid grid-cols-2 gap-4" style={{ background: 'var(--vscode-editor-background)', borderTop: '1px solid var(--vscode-panel-border)' }}>
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    ...
                                                    {clocks.map(c => (
                                                        <option key={c.name} value={c.name}>{c.name}{c.frequency ? ` (${c.frequency})` : ''}</option>
                                                    ))}
                                                </select>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    <option value="">None</option>
                                                    {resets.map(r => (
                                                        <option key={r.name} value={r.name}>{r.name}{r.polarity ? ` (${r.polarity})` : ''}</option>
                                                    ))}
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        {(() => {
                                            ...
                                            if (optionalPorts.length === 0) return null;
                                            ...
                                            return (
                                                <div className="px-4 py-2 text-sm" style={{ background: 'var(--vscode-editor-background)', borderTop: '1px solid var(--vscode-panel-border)' }}>
                                                    <div className="font-medium mb-2" style={TEXT_STYLES.label}>Optional Ports</div>
                                                    <div className="flex flex-wrap gap-3">
                                                        {optionalPorts.map(port => {
                                                            const isEnabled = enabledPorts.includes(port.name);
                                                            return (
                                                                <label key={port.name} className="flex items-center gap-1.5 cursor-pointer" title={`Width: ${port.width || 1}, Dir: ${port.direction || 'N/A'}`}>
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={isEnabled}
                                                                    ...
                                                                    />
                                                                    <span style={{ ...TEXT_STYLES.value, color: isEnabled ? undefined : 'var(--vscode-input-placeholderForeground)' }}>{port.name}</span>
                                                                </label>
                                                            );
                                                        })}
                                                    </div>
                                        ...
                                        <div className="px-4 py-2 text-sm" style={{ background: 'var(--vscode-editor-background)', borderTop: '1px solid var(--vscode-panel-border)' }}>
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="font-medium" style={TEXT_STYLES.label}>Array Configuration</span>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); toggleArray(index, !!bus.array); }}
                                                        ...
                                                        {editingArrayField?.busIndex === index && editingArrayField?.field === 'count' ? (
                                                            <div className="flex items-center gap-1">
                                                                <input
                                                                    ...
                                                                    style={{ ...TEXT_STYLES.value, background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', color: 'var(--vscode-input-foreground)', outline: 'none', fontSize: 'inherit', width: '50px' }}
                                                                    autoFocus
                                                                    min="1"
                                                                    onKeyDown={(e) => {
                                                                        e.stopPropagation();
                                                                        if (e.key === 'Enter') { e.preventDefault(); saveArrayField(index); }
                                                                        else if (e.key === 'Escape') { e.preventDefault(); cancelEditArrayField(); }
                                                                    }}
                                                                />
                                                                <button onClick={(e) => { e.stopPropagation(); saveArrayField(index); }} className="px-1 py-0.5 rounded text-xs" style={{ background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)' }}>✓</button>
                                                                <button onClick={(e) => { e.stopPropagation(); cancelEditArrayField(); }} className="px-1 py-0.5 rounded text-xs" style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-secondaryForeground)' }}>✗</button>
                                                            </div>
                                                        ) : (
                                                            <span onClick={() => startEditArrayField(index, 'count', bus.array!.count)} className="cursor-pointer" style={{ ...TEXT_STYLES.value, textDecoration: 'underline', textDecorationStyle: 'dotted' }} title="Click to edit">{bus.array.count}</span>
                                                        )}
                                                    </div>
                                                    {/* Index Start */}
                                                    <div className="flex items-center gap-2">
                                                        <span style={TEXT_STYLES.label}>Index Start:</span>
                                                        {editingArrayField?.busIndex === index && editingArrayField?.field === 'indexStart' ? (
                                                            <div className="flex items-center gap-1">
                                                                <input
                                                                    type="number"
                                                                    value={draftArrayValue}
                                                                ...
                                                                />
                                                                <button onClick={(e) => { e.stopPropagation(); saveArrayField(index); }} className="px-1 py-0.5 rounded text-xs" style={{ background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)' }}>✓</button>
                                                                <button onClick={(e) => { e.stopPropagation(); cancelEditArrayField(); }} className="px-1 py-0.5 rounded text-xs" style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-foreground)' }}>✗</button>
                                                            </div>
                                                        ) : (
                                                            <span onClick={() => startEditArrayField(index, 'indexStart', bus.array!.indexStart ?? 0)} className="cursor-pointer" style={{ ...TEXT_STYLES.value, textDecoration: 'underline', textDecorationStyle: 'dotted' }} title="Click to edit">{bus.array.indexStart ?? 0}</span>
                                                        )}
                                                    </div>
                                                    {/* Naming Pattern */}
                                                    <div className="col-span-2 flex items-center gap-2">
                                                        <span style={TEXT_STYLES.label}>Naming Pattern:</span>
                                                        {editingArrayField?.busIndex === index && editingArrayField?.field === 'namingPattern' ? (
                                                            <div className="flex items-center gap-1 flex-1">
                                                                <input
                                                                    ...
                                                                    style={{ ...TEXT_STYLES.value, background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', color: 'var(--vscode-input-foreground)', outline: 'none', fontSize: 'inherit' }}
                                                                    autoFocus
                                                            ...
                                                            <span onClick={() => startEditArrayField(index, 'namingPattern', bus.array!.namingPattern || '')} className="cursor-pointer" style={{ ...TEXT_STYLES.value, textDecoration: 'underline', textDecorationStyle: 'dotted', color: bus.array.namingPattern ? undefined : 'var(--vscode-input-placeholderForeground)' }} title="Click to edit">{bus.array.namingPattern || 'not set'}</span>
                                                        )}
                                                    </div>
                                                    {/* Physical Prefix Pattern */}
                                                    <div className="col-span-2 flex items-center gap-2">
                                                        <span style={TEXT_STYLES.label}>Prefix Pattern:</span>
                                                        {editingArrayField?.busIndex === index && editingArrayField?.field === 'physicalPrefixPattern' ? (
                                                            <div className="flex items-center gap-1 flex-1">
                                                                <input
                                                                    type="text"
                                                                    value={draftArrayValue}
                                                            ...
                                                            <span onClick={() => startEditArrayField(index, 'physicalPrefixPattern', bus.array!.physicalPrefixPattern || '')} className="cursor-pointer" style={{ ...TEXT_STYLES.value, textDecoration: 'underline', textDecorationStyle: 'dotted', color: bus.array.physicalPrefixPattern ? undefined : 'var(--vscode-input-placeholderForeground)' }} title="Click to edit">{bus.array.physicalPrefixPattern || 'not set'}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        {ports.length > 0 ? (
                                            <table className="w-full text-sm" style={{ borderTop: '1px solid var(--vscode-panel-border)' }}>
                                                <thead>
                                                    <tr style={{ background: 'var(--vscode-editor-background)' }}>
                                                        ...
                                                        <th className="px-4 py-2 text-left font-medium" style={TEXT_STYLES.label}>Presence</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {ports.map((port, pIdx) => {
                                                        const portSuffix = getPortSuffix(bus, port.name);
                                                        const physicalName = getPhysicalName(bus, port.name);
                                                        const isEditingThisPortName = editingPortName?.busIndex === index && editingPortName?.portName === port.name;
                                                        const isEditingThisPortWidth = editingPortWidth?.busIndex === index && editingPortWidth?.portName === port.name;
                                                        ...
                                                        const defaultWidth = busLibrary?.[bus.type]?.ports?.find(p => p.name === port.name)?.width || 1;
                                                            ...
                                                            <tr
                                                                ...
                                                                style={{
                                                                    ...
                                                                    background: isSelectedRow
                                                                        ...
                                                                        : pIdx % 2 === 0 ? 'transparent' : 'var(--vscode-editor-background)'
                                                                }}
                                                            >
                                                                <td className="px-4 py-1.5" style={TEXT_STYLES.value}>{port.name}</td>
                                                                <td
                                                                    className="px-4 py-1.5"
                                                                    style={{
                                                                        outline: isSelectedRow && selectedColumn === 'name' && !isEditingThisPortName ? '1px solid var(--vscode-focusBorder)' : undefined,
                                                                        outlineOffset: '-1px'
                                                                    }}
                                                                >
                                                                    {isEditingThisPortName ? (
                                                                        <div className="flex items-center gap-1">
                                                                            <span style={{ ...TEXT_STYLES.value, ...TEXT_STYLES.muted }}>{bus.physicalPrefix || ''}</span>
                                                                            <input
                                                                                type="text"
                                                                                value={draftPortName}
                                                                                onChange={(e) => setDraftPortName(e.target.value)}
                                                                                className="px-1 py-0.5 rounded"
                                                                                style={{ ...TEXT_STYLES.value, background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', color: 'var(--vscode-input-foreground)', outline: 'none', fontSize: 'inherit', minWidth: '80px' }}
                                                                                autoFocus
                                                                                onKeyDown={(e) => {
                                                                                    e.stopPropagation();
                                                                                    if (e.key === 'Enter') { e.preventDefault(); savePortName(index, port.name); }
                                                                                    else if (e.key === 'Escape') { e.preventDefault(); cancelEditPortName(); }
                                                                                }}
                                                                            />
                                                                            <button
                                                                                onClick={(e) => { e.stopPropagation(); savePortName(index, port.name); }}
                                                                                className="px-1.5 py-0.5 rounded text-xs"
                                                                                style={{ background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)' }}
                                                                            >✓</button>
                                                                            <button
                                                                                onClick={(e) => { e.stopPropagation(); cancelEditPortName(); }}
                                                                                className="px-1.5 py-0.5 rounded text-xs"
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span style={{ ...TEXT_STYLES.value, color: hasNameOverride ? 'var(--vscode-textLink-foreground)' : undefined, textDecoration: 'underline', textDecorationStyle: 'dotted' }}>{portSuffix}</span>
                                                                        </span>
                                                                    )}
                                                                </td>
                                                                <td
                                                                    className="px-4 py-1.5"
                                                                    style={{
                                                                        outline: isSelectedRow && selectedColumn === 'width' && !isEditingThisPortWidth ? '1px solid var(--vscode-focusBorder)' : undefined,
                                                                        outlineOffset: '-1px'
                                                                    }}
                                                                >
                                                                    {isEditingThisPortWidth ? (
                                                                        <div className="flex items-center gap-1">
                                                                            <input
                                                                                ...
                                                                                className="px-1 py-0.5 rounded"
                                                                                style={{ ...TEXT_STYLES.value, background: 'var(--vscode-input-background)', border: '1px solid var(--vscode-input-border)', color: 'var(--vscode-input-foreground)', outline: 'none', fontSize: 'inherit', width: '60px' }}
                                                                                autoFocus
                                                                                onKeyDown={(e) => {
                                                                                    e.stopPropagation();
                                                                                    if (e.key === 'Enter') { e.preventDefault(); savePortWidth(index, port.name, defaultWidth); }
                                                                                    else if (e.key === 'Escape') { e.preventDefault(); cancelEditPortWidth(); }
                                                                                }}
                                                                            />
                                                                            <button
                                                                                onClick={(e) => { e.stopPropagation(); savePortWidth(index, port.name, defaultWidth); }}
                                                                                className="px-1.5 py-0.5 rounded text-xs"
                                                                                style={{ background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)' }}
                                                                            >✓</button>
                                                                            <button
                                                                                onClick={(e) => { e.stopPropagation(); cancelEditPortWidth(); }}
                                                                                className="px-1.5 py-0.5 rounded text-xs"
                                                                                style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-foreground)' }}
                                                                            >✗</button>
                                                                        </div>
                                                                        ...
                                                                        <span
                                                                            ...
                                                                            className="cursor-pointer"
                                                                            style={{ color: hasWidthOverride ? 'var(--vscode-textLink-foreground)' : undefined, textDecoration: 'underline', textDecorationStyle: 'dotted' }}
                                                                ...
                                                                <td className="px-4 py-1.5">
                                                                    <span style={{ color: port.direction === 'in' ? 'var(--vscode-charts-green)' : port.direction === 'out' ? 'var(--vscode-charts-blue)' : undefined }}>
                                                                        {port.direction || '—'}
                                                                    </span>
                                                                </td>
                                                                <td className="px-4 py-1.5" style={TEXT_STYLES.muted}>{port.presence}</td>
                                                            </tr>
                                                        );
                                                    })}
                                                </tbody>
                                            ...
                                            <div className="px-4 py-4 text-sm" style={{ ...TEXT_STYLES.muted, borderTop: '1px solid var(--vscode-panel-border)' }}>
