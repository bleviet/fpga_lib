...
export const MetadataEditor: React.FC<MetadataEditorProps> = ({ ipCore, onUpdate }) => {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            const isTyping = target.closest('input, textarea, select, [contenteditable="true"], vscode-text-field, vscode-text-area, vscode-dropdown');
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm mt-1" style={{ opacity: 0.7 }}>
                        ...
                        <span className="ml-2 text-xs font-mono" style={{ opacity: 0.5 }}>j/k: navigate • e: edit</span>
                    </p>
                </div>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr style={{ background: 'var(--vscode-editor-background)', borderBottom: '1px solid var(--vscode-panel-border)' }}>
                            <th className="px-4 py-3 text-left text-sm font-semibold" style={{ width: '150px' }}>Field</th>
                            <th className="px-4 py-3 text-left text-sm font-semibold">Value</th>
                            <th className="px-4 py-3 text-right text-sm font-semibold" style={{ width: '80px' }}>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            return (
                                ...
                                >
                                    <td className="px-4 py-3 text-sm font-semibold" style={{ opacity: 0.8 }}>
                                        {row.label}
                                        {row.required && <span style={{ color: 'var(--vscode-errorForeground)' }}> *</span>}
                                    </td>
                                    <td className="px-4 py-3 text-sm">
                                        {isEditing ? renderEditField(row) : (
                                            <span className="font-mono" style={{ whiteSpace: row.type === 'textarea' ? 'pre-wrap' : 'normal' }}>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {!isEditing && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleEdit(row.key); }}
