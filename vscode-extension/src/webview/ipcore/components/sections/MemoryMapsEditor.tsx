...
export const MemoryMapsEditor: React.FC<MemoryMapsEditorProps> = ({ memoryMaps, imports, onUpdate }) => {
    ...
    return (
        <div className="p-6 space-y-4">
            ...
            {importFile ? (
                        ...
                        {detectedMaps.length > 0 ? (
                            <div className="space-y-3">
                                {detectedMaps.map((map: any, idx: number) => (
                                    <div key={idx} className="p-4 rounded border" style={{ background: 'var(--vscode-editor-background)', borderColor: 'var(--vscode-panel-border)' }}>
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-bold text-lg">{map.name}</span>
                                                    <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'var(--vscode-badge-background)', color: 'var(--vscode-badge-foreground)' }}>
                                                        {map.addressBlocks?.length || 0} Block{map.addressBlocks?.length !== 1 ? 's' : ''}
                                                    </span>
                                                </div>
                                                {map.description && (
                                                    <p className="text-sm mt-1" style={{ opacity: 0.8 }}>{map.description}</p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {map.addressBlocks && map.addressBlocks.length > 0 && (
                                            <div className="mt-3 pl-4 border-l-2" style={{ borderColor: 'var(--vscode-panel-border)' }}>
                                                <p className="text-xs font-semibold mb-1" style={{ opacity: 0.6 }}>ADDRESS BLOCKS</p>
                                                <div className="grid gap-1">
                                                    {map.addressBlocks.map((block: any, bIdx: number) => (
                                                        <div key={bIdx} className="text-sm font-mono flex items-center gap-3" style={{ opacity: 0.8 }}>
                                                            <span>{block.name}</span>
                                                            <span style={{ opacity: 0.5 }}>Offset: {block.offset || block.baseAddress || 0}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                            ...
                            <div className="p-8 text-center text-sm" style={{ opacity: 0.6, border: '1px dashed var(--vscode-panel-border)', borderRadius: '4px' }}>
                ...
                <div className="p-8 text-center" style={{ opacity: 0.6, border: '1px dashed var(--vscode-panel-border)', borderRadius: '4px' }}>
