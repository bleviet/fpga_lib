                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {parameters.map((param, index) => {
                            ...
                            return (
                                <tr key={index} {...rowProps} onDoubleClick={() => handleEdit(index)}>
                                    <td className="px-4 py-3 text-sm font-mono" {...getCellProps(index, 'name')}>{param.name}</td>
                                    <td className="px-4 py-3 text-sm" {...getCellProps(index, 'dataType')}>{param.dataType}</td>
                                    <td className="px-4 py-3 text-sm font-mono" {...getCellProps(index, 'defaultValue')}>{formatDefaultValue(param)}</td>
                                    <td className="px-4 py-3 text-sm" {...getCellProps(index, 'description')} style={{ ...getCellProps(index, 'description').style, opacity: param.description ? 1 : 0.5 }}>{param.description || '—'}</td>
                                    <td className="px-4 py-3 text-right">
                        ...
                        {parameters.length === 0 && !isAdding && (
                            <tr><td colSpan={5} className="px-4 py-8 text-center text-sm" style={{ opacity: 0.6 }}>No parameters defined. Press 'o' or click "Add Parameter".</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
