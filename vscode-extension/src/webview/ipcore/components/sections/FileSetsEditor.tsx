 ...
 */
export const FileSetsEditor: React.FC<FileSetsEditorProps> = ({ fileSets, onUpdate }) => {
    ...
    const handleDeleteSet = (idx: number) => {
        ...
        onUpdate(['fileSets'], fileSets.filter((_, i) => i !== idx));
    };
        ...
        <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    style={{
                        background: (isAddingSet || editingSet !== null) ? 'var(--vscode-button-secondaryBackground)' : 'var(--vscode-button-background)',
                ...
                {fileSets.map((fileSet, setIdx) => (
                    ...
                    >
                        ...
                        <div
                            ...
                            style={{ background: 'var(--vscode-sideBar-background)', borderBottom: expandedSets.has(setIdx) ? '1px solid var(--vscode-panel-border)' : 'none' }}
                            onClick={() => toggleSet(setIdx)}
                        >
                            <div className="flex items-center gap-3 flex-1">
                                <span className={`codicon codicon-chevron-${expandedSets.has(setIdx) ? 'down' : 'right'}`}></span>
                                <div>
                                    <p className="font-semibold">{fileSet.name || '(imported)'}</p>
                                    {fileSet.description && <p className="text-xs mt-1" style={{ opacity: 0.7 }}>{fileSet.description}</p>}
                                    {fileSet.import && <p className="text-xs mt-1 font-mono" style={{ opacity: 0.7 }}>Import: {fileSet.import}</p>}
                                </div>
                            </div>
                            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                <span className="text-xs px-2 py-1 rounded" style={{ background: 'var(--vscode-badge-background)', color: 'var(--vscode-badge-foreground)' }}>
                                    {fileSet.files?.length || 0} files
                                </span>
                                {!fileSet.import && (
                                    <>
                                        <button onClick={() => { setEditingSet(setIdx); setSetDraft({ name: fileSet.name, description: fileSet.description || '' }); }} className="p-1" title="Edit"><span className="codicon codicon-edit"></span></button>
                                        <button onClick={() => handleDeleteSet(setIdx)} className="p-1" style={{ color: 'var(--vscode-errorForeground)' }} title="Delete"><span className="codicon codicon-trash"></span></button>
                                    </>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="p-4 space-y-3">
                                ...
                                <div className="space-y-2">
                                    {(fileSet.files || []).map((file, fileIdx) => {
                                        const isEditing = editingFile?.setIdx === setIdx && editingFile?.fileIdx === fileIdx;
                                        ...
                                        return (
                                            <div key={fileIdx} className="flex items-center justify-between p-2 rounded hover:opacity-80" style={{ background: 'var(--vscode-input-background)' }}>
                                                <div className="flex items-center gap-3">
                                                    <span className="codicon codicon-file"></span>
                                                    <span className="text-sm font-mono">{file.path}</span>
                                                    <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'var(--vscode-badge-background)', color: 'var(--vscode-badge-foreground)' }}>{file.type}</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <button onClick={() => { setEditingFile({ setIdx, fileIdx }); setFileDraft(file); }} className="p-1" title="Edit"><span className="codicon codicon-edit"></span></button>
                                                    <button onClick={() => handleDeleteFile(setIdx, fileIdx)} className="p-1" style={{ color: 'var(--vscode-errorForeground)' }} title="Delete"><span className="codicon codicon-trash"></span></button>
                                                </div>
                                            </div>
                                        );
                                    })}
                                    ...
                                    {isAddingFile === setIdx && (
                                        <div className="flex items-center gap-2 p-3 rounded" style={{ background: 'var(--vscode-list-activeSelectionBackground)' }}>
                                            <button
                                            ...
                                            <button onClick={() => setIsAddingFile(null)} className="px-3 py-2 rounded text-sm" style={{ background: 'var(--vscode-button-secondaryBackground)', color: 'var(--vscode-button-foreground)' }}>Cancel</button>
                                        </div>
                                    )}
                                </div>
