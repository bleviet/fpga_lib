...
export interface UseVimTableNavigationReturn<T> {
    ...
    getCellProps: (rowIndex: number, columnKey: string) => {
        'data-col-key': string;
...
export function useVimTableNavigation<T>({
    ...
    }, [items.length, selectedIndex, activeColumn, editingIndex, isAdding, columnKeys, scrollToCell, focusActiveCell]);
    ...
    const getCellProps = useCallback((rowIndex: number, columnKey: string) => ({
        ...
        style: {
            outline: (selectedIndex === rowIndex && activeColumn === columnKey)
